            ...
            if doi != "None" and "doi" in ids:
                if doi in ids["doi"]:
                    ...
                    if DataSource == "biotools":
                        return name, label, languages, os, list_keywords
...
def create_InferedTools( c_para, conn_para):
    global c
    c = c_para
    global conn
    conn = conn_para
    ...
    for i in publications:
        ...
        print(counter)
        id_publication = i[0]
        name_tool, label, languages, operative_systems, list_keywords= search_json(data_json, str(i[1]), str(i[2]), str(i[3])) # Input the IDs of the publication from different platforms
        if not name_tool:
            continue
