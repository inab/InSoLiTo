...
# Create Open Access publication table
# PMCID: PMCID of the publication
c.execute('''DROP TABLE IF EXISTS OA_Publications''')
c.execute('''CREATE TABLE "OA_Publications" (
                "PMCID"	TEXT NOT NULL,
                PRIMARY KEY("PMCID")
            );''')

# Create Cache table for the Open Access database
# PMCID: PMCID of the publication
# Database: Name of the database
c.execute('''DROP TABLE IF EXISTS OA_Cache''')
c.execute('''CREATE TABLE "OA_Cache" (
                "PMCID"	TEXT NOT NULL,
...
for label in list_labels:
    ...
    # Create Citations table for each section
    # id1: Foreign key of publication
    # id2: Foreign key of the second publication
    # n_citations: Number of co-occurences between publications
    # year: Year when the co-occurence happenend.
    # id_pub: PMCID of the publication where the relationship happened
    c.execute(f'''DROP TABLE IF EXISTS Citations_{label}''')
    c.execute(f'''CREATE TABLE "Citations_{label}" (
                    "id1"	TEXT NOT NULL,
                    "id2"	TEXT NOT NULL,
                    "n_citations" INTEGER,
                    "year" INTEGER,
                    ...
                    FOREIGN KEY("id1") REFERENCES "Publications"("id"),
                    FOREIGN KEY("id2") REFERENCES "Publications"("id"),
                    FOREIGN KEY("id_pub") REFERENCES "Publications"("id")
                );''')
